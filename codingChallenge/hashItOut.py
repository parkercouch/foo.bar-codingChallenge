...
# is using. You already managed to steal the details of the algorithm used, and 
# with some quiet observation of the guards you find out the results of the 
# hash (the digest). Now to break it.
# 
# The function takes a 16 byte input and gives a 16 byte output. It uses m
# ultiplication (*), bit-wise exclusive OR (XOR) and modulo (%) to calculate an 
...
# created this digest. Since each value is a single byte, the values are 0 to 
# 255 for both message and digest.
...
def answer(digest):
	...
	for i in range(16):
		for x in range(256):
			message[i] = x
			if i == 0:
				check = (129 * message[i]) % 256
			else:
				check = ( (129 * message[i]) ^ message[i-1]) % 256
			if check == digest[i]:
				break
	return message